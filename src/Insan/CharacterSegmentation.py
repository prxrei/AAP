...
# app.py routing
OUTPUT_DIR = './Insan/segmented/'

#flask_backend.py routing
OUTPUT_DIR = './Insan/segmented/'

# if not os.path.exists(OUTPUT_DIR):
#     print(f"Directory {OUTPUT_DIR} does not exist. Creating now...")
...
def letter_seg(lines_img, x_lines, i):
    ...
    print(f"Found {len(contours)} contours in line {i + 1}.")
    ...
    print(f"Identified {len(letter_k)} valid letters in line {i + 1}.")
    ...
    for e in range(len(letter)):
        print(f"Processing letter {e + 1} in word {word}...")
        ...
        if letter[e][0] < x_linescopy[0]:
            ...
            output_path = os.path.join(OUTPUT_DIR, f'{i + 1}_{word}_{letter_index}.jpg')
            print(f"Saving letter image {output_path}")
            cv2.imwrite(output_path, 255 - letter_img)
        else:
            x_linescopy.pop(0)
            word += 1
            letter_index = 1
    ...
    print(f"Completed segmentation for line {i + 1}.")
...
def image_segmentation(filepath):
    ...
    print("\nStarting letter width calculation...")
    ...
    # Add more logs to check the flow here
    print("\nChecking for valid letter width...")
    if mean_lttr_width <= 0:
        print("Error: Invalid letter width detected.")
    else:
        print(f"Letter width calculated successfully: {mean_lttr_width}")

    # -------------/Letter Width Calculation-------#

    # --------------Word Detection-----------------#
    print("\nStarting word detection...")
    x_lines = []
